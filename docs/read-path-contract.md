...
| field | bucket | what it is |
| --- | --- | --- |
| `config` | durable | `configure`'s argument list, exactly as it was frozen onto the record. **`null` — explicitly, never omitted — when no record backs this row at all.** Every consumer of this block has to handle null rather than assuming a shape. **On `list_agents` it has no `prompt`** — see `promptChars` below and `configEchoContract.summarised`, which names the omission on the response itself; `agent_status` and `activate` carry it whole |
...
| `promptChars` | durable | how many characters the prompt on the record has. **`null` when the record carries none, and `null` when `config` is null** — read the two together, as with the rest of this block. On `list_agents` it is what stands in for the prompt text; on the surfaces that carry `config.prompt` whole it is that string's length, so the field means the same thing everywhere and a consumer never has to know which surface it is on before knowing whether the field exists. `0` and `null` are different answers: an empty prompt somebody froze, against no prompt at all ([KAN-528](https://wroosbit.atlassian.net/browse/KAN-528)) |
...
<a id="occupantowner"></a>
### `occupantOwner` — OccupantOwner

**On `activate_response` only** ([§8](#8-activate_response)), and only on the
`occupied` refusal. It answers *whose is the directory I was refused*, which the
refusal used to leave the reader to infer from the occupying pane's name.

**The value is the AGENT's, not the pane's.** The pane in the way is not ours,
so it has no record and carries no owner; what carries one is the agent whose
directory it is sitting in — the agent this call was asked to activate. Its
`owner` was frozen on by `configure` and is reported verbatim, matched **exactly**
and never interpreted. **Nothing here parses a pane name**, and that absence is a
design position rather than an oversight.

<!-- contract-table: BLOCK_SHAPES.OccupantOwner -->

| field | bucket | what it is |
| --- | --- | --- |
| `recorded` | durable | the `owner` on the agent's own record, verbatim, or `null` when it carries none |
| `askedAs` | derived | who the caller said it was on **this call**, or `null` when it did not say. Echoed exactly as sent |
| `relation` | derived | this daemon's comparison of the two — [occupantOwnerRelation](#occupantownerrelation) |

⚠ **`recorded` is `durable` and `askedAs` is not, and the split is why this is a
block rather than two flat fields.** `recorded` is read off the append-only
registry and means the same thing tomorrow. `askedAs` is this call's own input
handed back: on no record, from no census, and true of nothing but the request
that carried it. A consumer that caches it as a fact about the agent has cached
a fact about itself.

**Both inputs are carried beside the verdict deliberately.** `relation` is what a
program branches on, but a reader who wants to show an operator the two strings,
or who disagrees with the comparison, must not have to re-derive them from the
prose in `error`. They are also what makes the verdict auditable: `unowned`
beside a non-null `recorded` is a contradiction anybody can see, where a bare
verdict is one nobody can check.

<a id="occupantownerrelation"></a>
#### `occupantOwner.relation` — four members, and two of them are not answers

| value | what it means |
| --- | --- |
| `unowned` | **no owner is recorded** for the agent configured here. ⚠ **Not a claim that it is not yours** — every agent configured before anybody passed an owner carries none |
| `yours` | an owner is recorded and it **exactly equals** what the caller passed as `owner` — the caller's own agent, running under another runtime |
| `others` | an owner is recorded, the caller said who it was, and **they differ** |
| `unasked` | an owner is recorded and the caller **did not say** who it was, so the comparison was never made |

⚠ **`unowned` and `unasked` must never be folded into `others`.** Both are
questions nobody answered; `others` is an answer. The caller this refusal exists
for reads it and decides whether to stand a pane down, so a false NON-match here
does not over-report a listing — it stops an agent. `unowned` is also the
**majority** case on any fleet older than the `owner` field, so it is the branch
...
| `summarised` | derived | declared knobs **this** response carries as a measurement rather than whole, each an object of `knob`, `replacedBy`, `wholeAt` and `why`. A different axis from `drops`: that one is about a field nobody declared, this one about a declared field deliberately not carried here. `list_agents` answers one entry — `config.prompt` → the row's `promptChars` — because a prompt per row exceeds the socket's framing bound on a fleet of ten ([KAN-528](https://wroosbit.atlassian.net/browse/KAN-528)). `agent_status` answers `[]`, which is a different sentence from a missing block. **Never per-row:** no row is dropped and no total is reduced |
| `note` | derived | |
...
| branch | when | keys |
| --- | --- | --- |
| `live-session` | `success: true`, this daemon holds the session | `action` `success` `sessionless` `path` `paneName` `paneId` `sessionId` `createdAt` `status` `herdrStatus` `label` `configured` `state` `config` `configVersion` `configuredAt` `everActivated` `activatedBy` `promptChars` `channelEnabled` `provenance` `configEchoContract` |
| `sessionless` | `success: true`, no session of ours — every agent that outlived a daemon restart, and every stopped one | `action` `success` `sessionless` `path` `paneName` `paneId` `sessionId` `createdAt` `status` `workDir` `herdrStatus` `label` `configured` `state` `config` `configVersion` `configuredAt` `everActivated` `activatedBy` `promptChars` `channelEnabled` `provenance` `configEchoContract` |
| `no-record-no-pane` | `success: false` — nothing here has ever been an agent | `action` `success` `error` `path` `paneName` `configured` `state` `config` `configVersion` `configuredAt` `everActivated` `activatedBy` `promptChars` `channelEnabled` `provenance` `configEchoContract` |
...
| `occupantOwner` | derived, optional | whose the occupied directory is — [OccupantOwner](#occupantowner). **On the `occupied` refusal and nowhere else**, because no other branch asked |
...
| `occupied` | live panes here and **none of them ours** | `action` `success` `started` `error` `path` `refused` `verified` `occupiedBy` `occupantOwner` | — |
...
**This does not, by itself, earn the pty triple a section.** [The rule for what belongs here](#the-rule-for-what-belongs-here) says a response earns one when a caller outside this repository branches on it, and a named external consumer makes that test look satisfied. Whether it actually is was a **decision** rather than something to infer from this paragraph, and the section below is that decision, taken on [KAN-428](https://wroosbit.atlassian.net/browse/KAN-428).

#### The decision: the rule does not fire for the pty triple

**The rule does not fire here. The three stay uncovered, and this is a decision that was taken rather than a reading left to a later reader.** The alternative was real and was declined: a section of their own, declarations in `src/read-contract.ts`, reconciliation in `verify-read-contract.mjs` and a version bump. Three reasons, in the order they carry weight.

**The rule is about branching on a response's documented fields.** Every surface in the Covered table is one a consumer reads *structurally* — it looks at named fields and takes a different path depending on what it finds, which is exactly why a field changing shape underneath it is a breakage this contract can meaningfully promise not to cause. **The pty triple is a byte transport.** A terminal client opens a session, writes keystrokes into it and renders bytes back out; driving all three actions is not the same act as branching on the shape of any of them, and the test in the rule is the second one.

**And here is the reason that actually decides it: contracting the response shape would promise the wrong property.** What a terminal consumer leans on is not the shape of what comes back — it is the behaviour of the transport underneath: that output arrives in the order it was produced, that a live session keeps answering, that a session id goes on meaning the same session. **Not one of those is a field.** Publishing a shape here would put this contract's strongest guarantee over the one property nobody reads, while every property such a consumer genuinely depends on stayed exactly as uncontracted as it is today. That is **a green tick standing over the wrong property**, which is the defect this repository has recorded more often than any other, wearing a contract instead of a check. It would look like more safety and deliver none.

**The cost is real even where the benefit is not.** [The rule for what belongs here](#the-rule-for-what-belongs-here) is explicit that widening for its own sake is the compatibility-surface creep this contract was scoped against. A section is a standing promise that constrains every future change to these three actions, and buying one for a consumer that does not read what it would promise is a cost with nothing on the other side of it.

#### What would reopen this — a condition, not a sentiment

**If a consumer branches on a documented field of a pty response, the rule fires and the decision above is wrong.** That is the whole of the condition, and the half that matters is what it excludes: **consuming the surface does not reopen it.** Butchr's sidepanel drives all three actions today, that was known and on this page when the decision was taken, and it is why the question was asked rather than an argument for the other answer. Field-level branching is the test. Anything short of it leaves these rows where they are.

**This is the exposure the decision accepts, and it is written here rather than only in the ticket** so that whoever meets the condition can recognise it without knowing the history. If you are reading a pty response's documented fields and taking different paths on what you find, you are the case this decision did not cover — say so, and it is reopened.

**And the decision is checked against the world rather than merely recorded.** `scripts/verify-pty-consumer-named.mjs` asserts both halves in one verdict: that the paragraphs above are still here, **and** that `pty_init`, `pty_input` and `pty_resize` are still in `UNCOVERED_SURFACES` and in neither other boundary list. An edit that contracts the surface without revisiting this decision puts the document and the code into contradiction, and that is a red check rather than something a reader has to happen to notice.
