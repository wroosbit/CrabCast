  ...
  return {
    tools: [
      ...
      // so there is exactly one `daemon_status` and now two ways to reach it.
      {
        ...
        description:
          "Answers WHICH CRABCAST IS RUNNING — the question no amount of looking at the filesystem can answer, because the filesystem is the thing that lies. A daemon started before a rebuild goes on executing the old `dist/` while the checkout on disk has moved on and looks perfectly current; every filesystem check reads healthy and the fleet misbehaves anyway. Only the process knows what it was loaded from, and this is where it says so. Ask it when a fleet is behaving in a way the code you are reading does not explain. `build` is what the RUNNING PROCESS was built from, read out of the `dist/` it actually loaded: `commit` (never abbreviated), `clean` (whether that checkout was clean AT BUILD TIME, which is not a claim about now), `builtAt`, `gitRoot`, `distDir`, `stampPath`, `stampPresent` and `stampUsable`. THOSE LAST TWO ARE NOT THE SAME QUESTION: a stamp that is present and NOT usable means there is a file sitting in that directory naming a commit which this daemon refused to believe — because `dist/` was rewritten under it without re-stamping (`tsc` run by hand does exactly this), or because it is a format this daemon does not read. A confidently wrong provenance is worse than an absent one, so it is demoted rather than reported. `freshness` compares that against what is on disk RIGHT NOW, AND asks whether the commit it names was ever released: `state` is one of `current`, `process-predates-build` (the daemon is not running the build on disk — restart it), `build-predates-sources` (the build is older than `src/` — rebuild, then restart), `off-release-line` (KAN-592: the running build is entirely self-consistent and the commit it was built from is NOT reachable from this clone's release ref — a branch that never landed is deployed, and rebuilding will not clear it) or `unknown`, with a `summary` sentence naming the remedy, plus `processIsCurrentBuild`, `sourcesNewerThanBuild`, `onReleaseLine` and `basis`. ⚠ `current` NOW ASSERTS THREE THINGS AND NOT TWO — running the build on disk, that build newer than `src/`, and that build on the release line — because for 24 hours the first two were true of a fleet running an unmerged incident branch and `CURRENT` was what it printed. READ `onReleaseLine` AS THREE-VALUED: `true`, `false`, and `null` for COULD NOT TELL, which is NOT a `no` — no `git` on the machine, a `dist`-only install with no working tree, a clone holding none of the candidate refs, or a repository that does not contain the commit. A `null` demotes `state` to `unknown` rather than to `current`, deliberately. `releaseRef` names which local ref answered (`refs/remotes/origin/HEAD`, then `refs/remotes/origin/main`, then `refs/heads/main`), `releaseRefCommit` is the object the ancestry test was actually run against, `releaseRefCommittedAt` dates it, and `releaseLineRepo` is the tree it was read from. ⚠ NOTHING IN THIS ANSWER TOUCHES THE NETWORK AND NOTHING EVER MAY: the ref is whatever this clone last fetched. That does not weaken a `true` — reachability is monotonic under fast-forward, so a stale ref can only make a `true` LATE, never wrong — but it does weaken a `false`, which may name a commit that has landed on the remote since the last fetch. READ `basis`: `build-stamp` identifies the build, while `file-times` merely dates it and cannot tell 'nobody rebuilt this' from 'somebody rebuilt it and landed on the same newest mtime with the same file count'. `unknown` MAPS A FIELD NAME TO WHY IT IS NULL, everywhere on this response, and a non-empty `unknown` is NOT a clean bill of health — `state: 'unknown'` is deliberately not `current`, because a check that reports success when it could not run is worse than no check. WHAT THIS RESPONSE DOES NOT ESTABLISH, said plainly because the fields above read like a health check and are not one: a green `freshness` IS NOT A HEALTHY DAEMON. It says the running code matches the code on disk — nothing about whether that code is correct, whether the daemon can reach herdr, whether any agent is alive, or whether the fleet is doing what you asked. An agent that has silently stopped, a herdr that is not answering and a registry that failed to write all sit behind a `state: 'current'`. Use crabcast_list_agents for the fleet and crabcast_capacity for the machine; this call is about the process serving you and nothing else. `clean` is about build time and says nothing about your working tree now, and `commit` is what was BUILT, not what is checked out. It also carries the process facts: `pid`, `configPath`, `dataDir`, `registryPath`, `configuredAgents` (how many agents have records) and `expectedAgents` (how many of those records say activated — a COUNT OFF THE REGISTRY and not a census, so it is what SHOULD be running rather than what is; crabcast_list_agents' missingAgents is the difference). ⚠ READ `expectedStranded` BEFORE YOU READ `expectedAgents` AS A NUMBER OF STARTABLE AGENTS (KAN-619): it is how many of them have NO DIRECTORY TO RUN IN, so `expectedAgents` minus it is what can actually be started, and the remainder cannot be — activate refuses a path that does not resolve, there is no conversation left to resume, and `crabcast_forget_agent` is the only verb those records have. The count is NOT narrowed for you and that is deliberate: reconcile restores from the same unfiltered `activated` filter, so a narrowed count here would disagree with what the daemon actually attempts at every boot. `strandedTotal` beside it is a DIFFERENT POPULATION — every stranded record whatever its last event, so it also counts ones nobody ever activated — and it is therefore always `>=` `expectedStranded`; it is the same name, population and value crabcast_list_agents publishes, and the ONLY one of the two comparable against that response. Both are computed from the same expression crabcast_list_agents builds its strandedAgents rows from, so the two calls cannot disagree about one registry. A NON-ZERO `expectedStranded` IS NOT AN ERROR: it is the ordinary residue of workspaces somebody finished with and deleted without a forget. READ `unreadableRecordsTotal` BEFORE YOU BELIEVE THOSE TWO COUNTS: both are computed only from registry rows this daemon could PARSE, so a registry holding rows written by an older CrabCast reads `0 configured, 0 expected` — identical to an empty registry. When it is non-zero, `unreadableRecords` names each row: its `line` in the file, `problem` (`pre-migration`, `from-newer` or `unusable`), the `reason` naming what could not be read, `claimsPath` if it names a directory, and `raw`, the row's own bytes (a `prompt` is withheld and `promptRedacted` says so). THOSE AGENTS ARE NOT RUNNING AND NOTHING RESTORED THEM — but nothing was deleted either: the rows stay in the registry untouched and survive compaction, so a repair is editing the named line rather than discarding the file. The same two fields are on crabcast_list_agents. And `startedAt` and `bootId`, the same two fields crabcast_list_agents carries, from the same expression, so the two responses cannot disagree about when this daemon started. THE COST, because it is not free and this is where it is disclosed: `build` and `freshness` are recomputed on every call — two recursive directory walks, stat per file, over `dist/` and `src/`, plus THREE local `git` invocations for the release-line answer (`rev-parse --show-toplevel`, `for-each-ref`, `merge-base --is-ancestor`), each bounded at 10s so a repository on a wedged filesystem makes this call slow rather than hung — which is why they are here, on a call somebody makes deliberately, rather than on the fleet read a poller hits continuously. Do not poll this.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
  ...
  try {
    ...
    if (name === "crabcast_list_agents") {
      ...
      if ((after !== undefined || limit !== undefined) && !category) {
        throw new Error(
          ...
          "(missingAgents, preemptedAgents, standbyAgents, unstartedAgents, strandedAgents or foreignPanes)."
        );
      }
    ...
    if (name === "crabcast_daemon_status") {
      ...
      return {
        ...
        // caller their call broke when what it did was work. The same holds for
        // `off-release-line` (KAN-592), and it is worth saying rather than
        // leaving to the general rule: that state is the loudest thing this
        // response can say, which makes it the one somebody will reach to
        // promote to `isError`. It is still a call that worked.
        isError: res?.success === false,
      };
    }
